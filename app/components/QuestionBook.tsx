...
export default function QuestionBook() {
  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      try {
        ...
        const questionsRef = collection(db, 'users', firebaseUser.uid, 'questions');

        unsubscribeSnapshot = onSnapshot(
          questionsRef,
          (snapshot) => {
            const loadedQuestions: Question[] = snapshot.docs.map((questionDoc) => {
              const data = questionDoc.data();
              return {
                id: questionDoc.id,
                lesson: typeof data.lesson === 'string' ? data.lesson : 'Matematik',
                topic: typeof data.topic === 'string' ? data.topic : '',
                type: data.type === 'image' ? 'image' : 'text',
                content: typeof data.content === 'string' ? data.content : '',
                reason: typeof data.reason === 'string' ? data.reason : 'Diğer',
                status: data.status === 'resolved' ? 'resolved' : 'unresolved',
                date: typeof data.date === 'string' ? data.date : '',
              };
            });

            // Güvenli sıralama (ID'ye göre azalan)
            loadedQuestions.sort((a, b) => Number(b.id || 0) - Number(a.id || 0));

            setQuestions(loadedQuestions);
            setLoading(false);
          },
          (firebaseError) => {
            console.error('Sorular dinlenirken hata oluştu:', firebaseError);
            setError('Sorular Firebase üzerinden yüklenemedi.');
            setLoading(false);
          }
        );
      } catch (firebaseError) {
        console.error('Bağlantı kurulamadı:', firebaseError);
  ...
  const handleAddQuestion = async (e: React.FormEvent) => {
    ...
    if (!user) {
      alert('Soru kaydetmek için giriş yapmalısın.');
      return;
    }
    ...
    try {
      ...
      console.error("Soru Firebase'e kaydedilemedi:", firebaseError);
      setError(`Soru kaydedilemedi: ${firebaseError.message || 'Bilinmeyen hata'}`);
    } finally {
      setSaving(false);
    }
  };

  // Durum Değiştir (Çözüldü / Çözülmedi)
  const toggleStatus = async (id: string) => {
    ...
    const newStatus = question.status === 'unresolved' ? 'resolved' : 'unresolved';
  ...
  const deleteQuestion = async (id: string) => {
    ...
    try {
      ...
      console.error('Soru silinemedi:', firebaseError);
  ...
  // Resim sıkıştırma / okuma (Küçük boyutlu görseller için Base64 optimizasyonu)
  ...
  const filteredQuestions = questions.filter((q) =>
    activeTab === 'unresolved' ? q.status === 'unresolved' : q.status === 'resolved'
  );
    ...
    return (
      <div className="w-full max-w-5xl mx-auto flex items-center justify-center py-20">
        <div className="bg-white/90 rounded-3xl p-8 shadow-xl border border-amber-200 text-center">
          ...
          <p className="font-bold text-amber-950">Sorular yükleniyor...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="w-full max-w-5xl mx-auto flex items-center justify-center py-20">
        <div className="bg-white/90 rounded-3xl p-8 shadow-xl border border-amber-200 text-center">
          ...
          <h2 className="text-xl font-extrabold text-amber-950">Yapamadığım Sorular</h2>
          <p className="text-sm text-gray-500 mt-2">Sorularını görmek için giriş yapmalısın.</p>
        </div>
      </div>
    );
  }
